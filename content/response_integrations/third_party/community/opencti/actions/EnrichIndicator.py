...
from core.constants import ENRICH_INDICATOR_SCRIPT_NAME, INTEGRATION_NAME
...
SUCCESS_MESSAGE = ""
ERROR_MESSAGE = f"Error executing action {ENRICH_INDICATOR_SCRIPT_NAME}"
...
class EnrichIndicator(BaseAction):
    """Full enrichment of an Indicator (SDO) in OpenCTI.

    Searches for an Indicator whose **name** matches the entity value,
    retrieves its metadata (score, confidence, kill chain, validity window)
    and **all** relationships (Malware, TTPs, Campaigns, Threat Actors, etc.).
    """
    ...
    def _perform_action(self, _=None) -> None:
        ...
        for entity in suitable_entities:
            ...
            try:
                ...
                result = self.api_client.enrich_indicator(
                    identifier=identifier,
                )

                if not result.found:
                    self.logger.info(f"Indicator {entity_identifier} not found in OpenCTI")
                ...
                if result.relations:
                    self.soar_action.result.add_data_table(
                        title=f"Indicator Relations: {entity.identifier}",
        ...
        if successful_entities:
            ...
            output_parts.append(
                f"Successfully enriched the following indicators using "
                f"{INTEGRATION_NAME}:\n {', '.join(names)}\n"
            )
        ...
        if not_found_entities:
            output_parts.append(
                f"The following entities were not found as indicators in "
        ...
        if not successful_entities:
            self.output_message = "No indicators were found in OpenCTI for the provided entities."
...
def main() -> None:
    """Entry point for the *Enrich Indicator* action."""
    EnrichIndicator(ENRICH_INDICATOR_SCRIPT_NAME).run()
